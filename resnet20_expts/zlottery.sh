...
BASE_PRUNE_METHOD="zprune_global_20"



function set_prune_method() {
    RATIO="$(python -c "print('{:.3f}'.format(100*.8 ** $1))")"
    PRUNE_METHOD="zprune_all_to_global_${RATIO}"
}
...
for idx in `seq 0 ${PRUNE_ITERATIONS}`; do
    ...
    if [ "$idx" -eq 0 ]; then
        ...
        if is_first_attempt; then
            ...
            run_or_debug gsutil -m cp "${FS_PREFIX}/execution_data/${PREV_NAME}/graph.pbtxt" "${FS_PREFIX}/execution_data/${CURR_NAME}/"

            TEMP_CHECKPOINT="$(mktemp)"
            echo 'model_checkpoint_path: "'"checkpoint_iter_${PRUNE_INDEX}"'"' > "${TEMP_CHECKPOINT}"
            gsutil cp "${TEMP_CHECKPOINT}" "${FS_PREFIX}/execution_data/${CURR_NAME}/checkpoint"
            rm "${TEMP_CHECKPOINT}"
        fi
    ...
    run_or_debug "$(dirname $0)/run_base.sh" "${CURR_NAME}" --max_train_steps 71108 --lottery_pruning_method "${PRUNE_METHOD}" --lottery_checkpoint_iters "${PRUNE_INDEX}" "${extra_params[@]}"
done
