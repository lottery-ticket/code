...
FINETUNE_EPOCHS="$1"
TRIAL_INDEX="$2"
TRAIN_STEPS="$(expr ${FINETUNE_EPOCHS} \* \( 50000 / 128 \) || :)"
...
for idx in `seq 0 ${PRUNE_ITERATIONS}`; do
    ...
    run_or_debug "$(dirname $0)/run_base.sh" "$(name_of_seq $idx)" --lottery_pruning_method "${PRUNE_METHOD}" --max_train_steps "${TOTAL_TRAIN_STEPS}" "${extra_params[@]}"
