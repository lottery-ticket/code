...
BASENAME="${NETWORK}/${BASE_PRUNE_METHOD}/${VERSION}/random_lottery/prune_${PRUNE_INDEX}/trial_${TRIAL_INDEX}"
...
for idx in `seq 0 ${PRUNE_ITERATIONS}`; do
    ...
    run_or_debug "$(dirname $0)/run_base.sh" "${CURR_NAME}" --train_steps 112590 --lth_use_random_label=true --skip_host_call=false --lottery_pruning_method "${PRUNE_METHOD}" --lottery_checkpoint_iters "${PRUNE_INDEX}" "${extra_params[@]}"
done
