...
# ==============================================================================
"""Contains utility and supporting functions for VGG.

  This module contains VGG code which does not directly build layers. This
includes dataset management, hyperparameter and optimizer code, and argument
parsing. Code for defining the VGG layers can be found in vgg_model.py.
"""
...
from official.vgg import vgg_model
...
def vgg_model_fn(features, labels, mode, model_class,
                 vgg_size, weight_decay, learning_rate_fn, momentum,
                 data_format, loss_scale,
                 loss_filter_fn=None, dtype=tf.float32,
                 fine_tune=False):
  """Shared functionality for different vgg model_fns.

  Initializes the VggModel representing the model layers
  ...
  Args:
    ...
    model_class: a class representing a TensorFlow model that has a __call__
      function. We assume here that this is a subclass of VggModel.
    vgg_size: A single integer for the size of the VGG model.
  ...
  model = model_class(vgg_size, data_format, dtype=dtype)
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    learning_rate = lottery.get_lr_tensor(flags.FLAGS.flag_values_dict())
