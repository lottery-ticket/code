...
cur = conn.execute('SELECT * FROM iters where path like "%vgg_16_nofc/prune_global_20_fc/v3%"')
...
for (it, expts) in sorted(iters.items()):
    ...
    plt.legend()
    plt.tight_layout()
    plt.title('VGG-16 $\Delta$ test accuracy after {idx}{suffix} pruning iteration ({sparsity:.2%} sparsity)'.format(
        idx=it,
    ...
    if plot_utils.save():
        plt.savefig(os.path.join('results_iterative', 'vgg16_{}'.format(it) + '.png'))
