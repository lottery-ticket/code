...
other = conn.execute('select * from iters where path like "s3://REDACTED-data/results/vgg_19_nofc/prune_global_20_fc/v1/lottery/prune_4701/trial_%/iter_0"')
trial_map = {
    path.split('/')[-2]: acc
    for (path, _, acc) in other.fetchall()
}

cur = conn.execute('SELECT * FROM iters where path like "%vgg_19_nofc/prune_global_%/v4%"')
...
for (it, expts) in sorted(iters.items(), key=lambda x: float(x[0].split('_')[-1])):
    ...
    plt.legend()
    plt.title('VGG-19 $\Delta$ test accuracy after one-shot prune to ${}\%$ sparsity'.format(it.split('_')[-1]))
    ...
    if plot_utils.save():
        plt.savefig(os.path.join('results_oneshot', 'vgg19_' + it + '.png'))
